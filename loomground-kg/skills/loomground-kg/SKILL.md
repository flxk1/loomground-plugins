---
name: loomground-kg
description: The cockpit over the Loomground Versum knowledge graph. Use whenever the user wants to see the state of the KG, ask what a source grounds or what grounds a claim, check whether this week's digest/signals are captured in the graph, decide what to run next across the Loomground skill platforms, or route work (draft a newsletter, build a lesson, capture a source) grounded in the graph. Triggers: "KG status", "what's in the knowledge graph", "is this week's digest grounded", "what should I run next", "which sources cover X", "cockpit", "route this to the right skill", "what does <source> ground", "coverage of <domain>".
---
...
- `search <term> [--limit N]` — sources whose claims mention a term (a thin models → sources
...
- **The concept/mental-model layer is not curated yet** (`status` shows `concepts: 0`). Until
  the coordinate-identity curation runs, "which *models* does this source ground / which
  sources ground this *model*" is answered at the **claim/source level** via `search`, not by
  true concept nodes. Say so when it matters.
